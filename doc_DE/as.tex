...
{\em G"ultigkeit: PowerPC, M-Core, XGate, 4004/4040, MCS-48/(2)51, 8086,
     ...
     M16C, H8/300, H8/500, SH, H16, 8080/8085, Zx80, i960, XA, 29K,
...
\begin{cpulist}
   ...
   SH7780 (SH4A) \\
\end{cpulist}
Es gab sehr viele Varianten von CPUs und MPUs mit SH-Kern.  Diese Targets
sind Stellvertreter f"ur die verschiedenen Generationen von Kernen
und die optionalen Funktionseinheiten.  Der SH2-Kern bietet eine
Handvoll Befehle mehr, die L"ucken im Befehlssatz des SH1 schlie"sen
(verz"ogerte, bedingte sowie relative und indirekte Spr"unge,
Multiplikationen mit 32-Bit-Operanden sowie Multiplizier/Addier-Befehle).
Der SH3-Kern erg"anzt eine zweite Registerbank, bessere Schiebebefehle
sowie Befehle zur Cache-Steuerung.  SH2A ist eine 'Seitenentwicklung'
mit Befehlserweiterungen, die nur dort vorhanden sind (Register-indirekte
Adressierung mit 12 Bit Displacement, Bitoperationen).
%%-----------
\begin{cpulist}
...
\item{G"ultigkeit SUPMODE: 680x0, NS32xxx, CR16C, PDP-11, i960,
      TLCS-900, SH, i960, 29K, Z280, XA, PowerPC, M*CORE,
      V60 und TMS9900}
\item{G"ultigkeit FPU: 680x0, NS32xxx, 80x86, WE32xxx, SH}
...
{\em G"ultigkeit: 680x0, 68xx, M*Core, XA, H8, SH, TMS9900,\\
...
{\em G"ultigkeit: 680x0, M*Core, 68xx, H8, SH, DSP56xxx,\\
...
\subsection{DN,DB,DW,DD,DQ,DT,DO \& BF16}
\ttindex{DN}\ttindex{DB}\ttindex{DW}\ttindex{DD}
\ttindex{DQ}\ttindex{DT}\ttindex{DO}\ttindex{BF16}
...
\item{\tty{BF16}: Gleitkomma im BFloat16-Format}
\end{itemize}
...
{\em G"ultigkeit: SH, IM61x0, IMP-16, IPC-16}

Da der SH-Prozessor seine Register immediate nur mit 8-Bit-Werten
...
SH-spezifischen Dingen hingewiesen.
...
hervorrufen...).  Will man beim SH PC-relativ adressieren, so tut man
...
\item{\tty{byte-swap}: erlaubt es, die Bytes in der Bin"ardatei umzusortieren.
      Mit einer \tty{2} als Argument werden die Bytes paarweise getauscht,
      mit einer \tty{4} werden Vierergruppen gespiegelt.}
...
\errentry{ 140}{Nicht vorhersagbare Ausf"uhrung dieser Anweisung}
               ...
               {keines}
\errentry{ 141}{widerstreitende Benutzung des gleichen Ziels in parallelem Konstrukt}
               {Warnung}
               {Zwei Anweisungen in einem parallelen Konstrukt verwenden
                das gleiche Zielregister.  Das Ergebnis davon ist undefiniert.}
               {das doppelt verwendete Register}
...
\errentry{1011}{nicht gen"ugend vorherige lokale Symbole}
               {Fehler}
               {Ein r"uckw"arts referenziertes lokales Symbol
                dieser Tiefe existiert nicht.}
               {Das nicht gefundene Symbol}
...
\errentry{1355}{Adressierungsmodus im Parallelbetrieb nicht erlaubt}
               ...
               {keines}
\errentry{1356}{ung"ultiges paralleles Konstrukt}
               {Fehler}
               {Die parallele Ausf"uhrung dieser Instruktionen
                ist nicht m"oglich.}
               {keines}
\errentry{1360}{undefinierte Bedingung}
               ...
               {Die benutzte Bedingung f"ur bedingte Spr"unge
                existiert nicht.}
               {keines}
\errentry{1364}{Befehl kann nicht bedingt ausgef"uhrt werden}
               {Fehler}
               {Dieser Maschinenbefehl kann nicht mit einer
                Bedingung verkn"ufpt werden.}
               {der fragliche Maschinenbefehl}

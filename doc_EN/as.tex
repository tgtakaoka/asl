...
\begin{cpulist}
   ...
   SH7780 (SH4A) \\
\end{cpulist}
There have been many variants of CPUs and FPUs with SH cores.  These
targets represent the various generations of cores with associated
optional function units.  The SH2 core offers a few more instructions that
close gaps in the SH1's instruction set (delayed conditional and
relative and indirect jumps, multiplications with 32-bit operands and
multiply/add instructions).  The SH3 core furthermore offers a second
register bank, better shift instructions, and instructions to control
the cache.  SH2A is a 'side development' with instruction extensions
offered nowhere else (register indirect addressing with 12 bit displacement,
bit operations).
%%-----------
\begin{cpulist}
...
\item{SUPMODE valid for: 680x0, NS32xxx, CR16C, PDP-11, i960,
      ...
      V60, and TMS9900}
\item{FPU valid for: 680x0, NS32xxx, 80x86, WE32xxx, SH}
...
{\em valid for: 680x0, 68xx, M*Core, XA, H8, SH, MSP430(X), TMS9900,\\
...
{\em valid for: 680x0, M*Core, 68xx, H8, SH, DSP56xxx, XA,
...
{\em valid for: 680x0, M*Core, 68xx, H8, SH, DSP56xxx, XA, ST7/STM8,
...
\subsection{DN,DB,DW,DD,DQ,DT,DO and BF16}
...
\item{\tty{BF16}: floating point in bfloat16 format}
\end{itemize}
...
{\em valid for: SH, IM61x0, IMP-16, IPC-16}

Although the SH processor can do an immediate register load with
...
PC-relative addressing on the SH, simply use ''absolute''
...
\item{\tty{byte-swap}: allows to re-sort bytes in the
      binary file.  A \tty{2} as argument performs a pairwise
      swap, while a \tty{4} swaps quartets of bytes.}
...
\errentry{ 140}{unpredictable execution of this instruction}
               ...
               {The addressing mode used for this instruction is allowed,
                ...
                instruction.}
               {none}
\errentry{ 141}{conflicting usage of same destination in parallel construct}
               {warning}
               {Two instructions in this parallel construct use the same
                register as destination.  The result of this is undefined.}
...
\errentry{1011}{not enough previous local symbols}
               {error}
               {A backward-referenced local symbol of this
                name does not exist.}
...
\errentry{1355}{addressing mode not allowed in parallel operation}
               ...
               {none}
\errentry{1356}{invalid parallel construct}
               {error}
...
\errentry{1360}{undefined condition}
               ...
               {The branch condition used for a conditional jump does not
                exist.}
               {none}
\errentry{1364}{no conditional execution of this instruction}
               {error}
               {This machine instruction cannot be combined with a
                condition.}
               {the machine instruction in question}
